...
def make_graph(frame_count, total_data_number):
    ...
    for i in range(96):
        ...
        if len(look_df["R+G+B Value"]) != 0: #要素が存在するのであれば
            ...
            x_data = look_df["R+G+B Value"].values #xのデータとしてR+G+B成分値
            y_data = look_df["R-B Value"].values #yのデータとしてR-B成分値

            #a, b = np.polyfit(x_data, y_data, 1) #近似直線を作るための傾きと切片を出す
            a = clf.fit(x, y)
            b = clf.intercrpt_

            #yの標本分散
            sy2 = look_df["R-B Value"].var(ddof=False)

            d = look_df["R-B Value"] - a * look_df["R+G+B Value"] - b
            syx2 = np.mean(d**2)
            sr2 = sy2 - syx2
            r2 = sr2 / sy2
            ...
            pyplot.legend(["密度:" + f'{density:.3f}' + "%", "近似式:" + approximate_formula], "決定係数: + "f'{r2:.3f}')
